...
@testset "bc tracer support" begin
    ...
    bc_inj = FlowBoundaryCondition(1, 1.5e5;
    ...
    acc .= 0.0
    bc_prod = FlowBoundaryCondition(1, 5.0e4;
        fractional_flow = [1.0, 0.0],
        density = 1000.0,
        tracers = [0.1],
        trans_flow = 2.0
    ...
    x = zeros(Jutul.vectorization_length(bc_inj, model, :bc, :all))
    cfg = Jutul.vectorize_force!(x, model, bc_inj, :bc, :all)
    bc_roundtrip = Jutul.devectorize_force(bc_inj, model, x, cfg, :bc, :all)
    @test Tuple(bc_roundtrip.tracers) == bc_inj.tracers
end
