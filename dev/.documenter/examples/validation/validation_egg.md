...
# The Egg model: Two-phase oil-water model {#The-Egg-model:-Two-phase-oil-water-model}

A two-phase model that is taken from the first member of the EGG ensemble. The model is a synthetic case with channelized permeability and water injection with fixed controls. For more details, see the paper where the ensemble is introduced:
...
ReservoirSimResult with 162 entries:
  ...
  wells (12 present):
    ...
    Results per well:
       ...
       :wcut => Vector{Float64} of size (162,)
  ...
  result (extended states, reports)
     SimResult with 135 entries

  extra
     Dict{Any, Any} with keys :simulator, :config

  Completed at May. 20 2025 23:05 after 30 seconds, 284 milliseconds, 892.6 microseconds.
```
...
## Load reference solution (OPM Flow) {#Load-reference-solution-OPM-Flow}
...
## Well responses and comparison {#Well-responses-and-comparison}

As the case is a two-phase model with water injection, we limit the results to plots of the producer water and oil rates.
...
If you would like to run this example yourself, it can be downloaded from the JutulDarcy.jl GitHub repository [as a script](https://github.com/sintefmath/JutulDarcy.jl/blob/main/examples/validation/validation_egg.jl), or as a [Jupyter Notebook](https://github.com/sintefmath/JutulDarcy.jl/blob/gh-pages/dev/final_site/notebooks/validation/validation_egg.ipynb)

```
This example took 51.17187325 seconds to complete.
```
