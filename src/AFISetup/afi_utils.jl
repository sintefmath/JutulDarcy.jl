...
function obsh_to_summary(obsh, t_seconds = missing;
        start_date = missing,
        smooth = false,
        alpha = 0.7,
...
function smooth_interpolant(I::Jutul.LinearInterpolant, alpha::Float64; sentinel = missing)
    vals = I.F
    smoothed_vals = exponential_smoothing(vals, alpha, sentinel)
    return Jutul.get_1d_interpolator(I.X, smoothed_vals, cap_endpoints = true)
end

function exponential_smoothing(data::Vector{Float64}, alpha::Float64, sentinel = missing)
